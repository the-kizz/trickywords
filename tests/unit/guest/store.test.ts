...
describe('guest store', () => {
  ...
  it('round-trips guest progress', () => {
    ...
    saveGuest({ avatar: 'owl', progress: { said: p }, bestKnown: 2, lastSetId: null, schoolSetId: null, grownUp: null, startedAt: 5 })
    const s = loadGuest()
  ...
  it('clears everything for the next child on a shared device', () => {
    saveGuest({ avatar: 'bee', progress: { a: newProgress('a') }, bestKnown: 0, lastSetId: null, schoolSetId: null, grownUp: null, startedAt: 1 })
    clearGuest()
